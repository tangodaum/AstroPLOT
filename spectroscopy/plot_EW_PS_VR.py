...
def splot(star, suffix, ref0, ref1, files, fmt, shift, cmap,
    ...
    for i in range(len(files)):
        ...
        # *****
        plt.plot(lbdas[i], specs[i], color=cor[i],
                 label=('%s-%s-%s' % (year[i], month[i], day[i])))
...
def read_harps(fname):
    try:
        ...
        # to_u = 1e-4
        # spec = fitsio.read(fname)
        # # spec = np.reshape(spec, np.size(spec))
        # header = fitsio.read_header(fname)
        # lbda = np.arange(np.size(spec)) * 0.01 + 3781.57
        # lbda = lbda * to_u  # to micronsa
        # mjd = header['MJD-OBS']
        # spec_total = []
        # first = 3781.57
        # for i in range(len(spec)):
        #     x_tmp = np.arange(len(spec[i])) * 0.01 + first
        #     inf = 100
        #     sup = 600
        #     t1 = x_tmp[inf:sup]
        #     t2 = x_tmp[-(sup + 1):-(inf + 1)]
        #     x = np.concatenate([t1, t2])
        #     spec_adj1 = spec[i][inf:sup]
        #     spec_adj2 = spec[i][-(sup + 1):-(inf + 1)]
        #     spec_adj = np.concatenate([spec_adj1, spec_adj2])
        #     coef = np.polyfit(x, spec_adj, 3)
        #     adj = coef[0] * x_tmp**3 + coef[1] * x_tmp**2 + coef[2] * x_tmp +\
        #         coef[3]
        #     norm = spec[i] / adj
        #     spec_total = np.concatenate([spec_total, norm])
        #     first = np.copy(x[-1])
        # return lbda, spec_total, mjd
...
# ==============================================================================
def plot_subplot_old(x, y, pos, ylabel, cor, gs):
...
def plot_subplot(x, y, pos, ylabel, cor, gs):
    ...
    plt.legend(fontsize=10, loc=4, frameon=False)
    plt.minorticks_on()
    leg = plt.legend(fontsize=10, loc=4, frameon=False)
...
plot_subplot(x=mjd_1, y=pksp / 1000, pos=0, ylabel=r'$Pk. sep. \, [km/s]$',
             cor=cor, gs=gs1)
plot_subplot(x=mjd_2, y=EW, pos=1, ylabel=r'$EW \, [\AA]$', cor=cor, gs=gs1)
